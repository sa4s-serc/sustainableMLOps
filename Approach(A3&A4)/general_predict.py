...
try:
    regressor = load('path/to/Models/regressor_model.pkl')
...
def predict_aqi_general(input_sequence):
    try:
        ...
        global regressor
        flag = pd.read_csv('path/to/Model_reload/reload_flag_general.csv', header=None)
        reload_flag = flag.iloc[0, 0]
        if (reload_flag == 'reload'):
            regressor = load('path/to/Models/regressor_model.pkl')
            print("General model reloaded")
            pd.DataFrame(['none']).to_csv('path/to/Model_reload/reload_flag_general.csv',
