...
def monitor_for_drift():
    ...
    time.sleep(300)
    while True:
        model = pd.read_csv('Model_reload/model.csv', header=None)
        model_type = model.iloc[0, 0]
        baseline_file = 'Knowledge/train_results_LSTM.csv' if model_type == 'lstm' else 'Knowledge/train_results_general_model.csv'
        baseline_file_lstm = 'Knowledge/train_results_LSTM.csv'
        baseline_file_general = 'Knowledge/train_results_general_model.csv'
        real_time_file = 'Results/prediction_results.csv'
...
def monitor_emissions_and_switch_model():
    ...
    while True:
        ...
        # print(len(skiprows))
        predictions = load_predictions('Results/prediction_results.csv',
                                       skiprows=skiprows)
        if predictions is not None and not predictions.empty:
            new_data_count = len(predictions)
            # if 'CPU Usage' in predictions.columns:
            #     analyser_instance.analyze_emissions(predictions)
